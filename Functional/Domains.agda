...
module _ {ℓₗ ℓₒ₁ ℓₒ₂ : Lvl.Level} {X : Type{ℓₒ₁}} {Y : Type{ℓₒ₂}} where
  open Logic.Predicate{ℓₗ}
  open Relator.Equals{ℓₗ Lvl.⊔ ℓₒ₁}
  open Relator.Equals.Proofs{ℓₗ Lvl.⊔ ℓₒ₁}
  ...
  record ⊶_ (f : X → Y) : Type{ℓₗ Lvl.⊔ ℓₒ₁ Lvl.⊔ ℓₒ₂} where
    constructor intro
    field
      obj : Y
      ⦃ .proof ⦄ : ∃(x ↦ f(x) ≡ obj)
  ...
  .[⊶]-arg : ∀{f : X → Y} → (⊶ f) → X
  [⊶]-arg{f} (intro _ ⦃ proof ⦄) = Lang.Irrelevance.axiom([∃]-witness (proof))
  ...
  -- .[⊶]-function-surjective : ∀{f : X → Y}{y : (⊶ f)} → ∃(x ↦ (([⊶]-fn f)(x) ≡ y))
  ...
  -- (Unmap f(y)) is also called "the fiber of the element y under the map f".
  -- Unmap(f) is similar to the inverse image or the preimage of f when their argument is a singleton set.
  record Unmap {X : Type{ℓₒ₁}} {Y : Type{ℓₒ₂}} (f : X → Y) (y : Y) : Type{ℓₗ Lvl.⊔ ℓₒ₁} where
    constructor intro
    field
      obj : X
      ⦃ proof ⦄ : (f(obj) ≡ y)
