...
module _ where
  ...
  record Regular ⦃ _ : Equiv{ℓₑ}(T) ⦄ (_+_  : T → T → T) (_⋅_  : T → T → T) ⦃ rg : Rg(_+_)(_⋅_) {ℓₙ₀} ⦄ : Stmt{Lvl.of(T) Lvl.⊔ ℓₑ} where
    constructor intro
    open Rg(rg)
    field
    ...
    zero-zero-divisorₗ {x} ([∃]-intro y ⦃ [∧]-intro y𝟎 xy𝟎 ⦄) = [∨]-elim id ([⊥]-elim ∘ [↔]-to-[→] NonZero.proof y𝟎) (no-zero-divisors xy𝟎)

    zero-zero-divisorᵣ : ZeroDivisorᵣ(x) → (x ≡ 𝟎)
    zero-zero-divisorᵣ {x} ([∃]-intro y ⦃ [∧]-intro y𝟎 xy𝟎 ⦄) = [∨]-elim ([⊥]-elim ∘ [↔]-to-[→] NonZero.proof y𝟎) id (no-zero-divisors xy𝟎)

    zero-zero-divisor : ZeroDivisor(x) → (x ≡ 𝟎)
    zero-zero-divisor {x} ([∃]-intro y ⦃ [∧]-intro y𝟎 ([∧]-intro xy𝟎 yx𝟎) ⦄) = [∨]-elim id ([⊥]-elim ∘ [↔]-to-[→] NonZero.proof y𝟎) (no-zero-divisors xy𝟎)

  -- Non-trivial ring that has no non-zero zero divisors.
  record Domain ⦃ _ : Equiv{ℓₑ}(T) ⦄ (_+_  : T → T → T) (_⋅_  : T → T → T) {ℓₙ₀} : Stmt{Lvl.of(T) Lvl.⊔ ℓₑ Lvl.⊔ Lvl.𝐒(ℓₙ₀)} where
    constructor intro
  ...
  record IntegralDomain ⦃ _ : Equiv{ℓₑ}(T) ⦄ (_+_  : T → T → T) (_⋅_  : T → T → T) {ℓₙ₀} : Stmt{Lvl.of(T) Lvl.⊔ ℓₑ Lvl.⊔ Lvl.𝐒(ℓₙ₀)} where
    constructor intro
    field ⦃ ring ⦄ : Ring(_+_)(_⋅_) {ℓₙ₀}
    open Ring(ring) public
    field ⦃ regular ⦄ : Regular(_+_)(_⋅_)
...
record IntegralDomainObject : Stmt{Lvl.𝐒(ℓ Lvl.⊔ ℓₑ Lvl.⊔ ℓₙ₀)} where
  constructor intro
  field
    {T} : Type{ℓ}
    ⦃ equiv ⦄ : Equiv{ℓₑ}(T)
    _+_ : T → T → T
    _⋅_ : T → T → T
    ⦃ integralDomain ⦄ : IntegralDomain(_+_)(_⋅_) {ℓₙ₀}
