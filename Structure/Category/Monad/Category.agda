...
Function.congruence (ExtensionSystem.ext-function extension-system) xy = [≡]-with2ᵣ(_∘_)(_) ([≡]-with(map) xy)
...
ExtensionSystem.ext-identity extension-system {x} {y} {f} =
  ...
  μ(y) ∘ (map(f) ∘ η(x)) 🝖-[ [≡]-with2ᵣ(_∘_)(_) (symmetry(_≡_) η-natural) ]
  μ(y) ∘ (η(T(y)) ∘ f)   🝖-[ symmetry(_≡_) (Morphism.associativity(_∘_)) ]
  (μ(y) ∘ η(T(y))) ∘ f   🝖-[ [≡]-with2ₗ(_∘_)(_) (_⊜_.proof μ-functor-[∘]-identityᵣ) ]
  id ∘ f                 🝖-[ Morphism.identityₗ(_∘_)(id) ]
  f                      🝖-end
ExtensionSystem.ext-distribute extension-system {x} {y} {z} {f} {g} =
  ext(ext(f) ∘ g)                               🝖[ _≡_ ]-[]
  μ(z) ∘ map((μ(z) ∘ map(f)) ∘ g)               🝖-[ [≡]-with2ᵣ(_∘_)(_) op-preserving ]
  μ(z) ∘ (map(μ(z) ∘ map(f)) ∘ map(g))          🝖-[ [≡]-with2ᵣ(_∘_)(_) ([≡]-with2ₗ(_∘_)(_) op-preserving) ]
  μ(z) ∘ ((map(μ(z)) ∘ map(map(f))) ∘ map(g))   🝖-[ associate4-231-123(cat) ]
  ((μ(z) ∘ map(μ(z))) ∘ map(map(f))) ∘ map(g)   🝖-[  [≡]-with2ₗ(_∘_)(_) ( [≡]-with2ₗ(_∘_)(_) (_⊜_.proof μ-functor-[∘]-commutativity)) ]
  ((μ(z) ∘ μ(T(z))) ∘ map(map(f))) ∘ map(g)     🝖-[ associate4-123-213(cat) ]
  (μ(z) ∘ (μ(T(z)) ∘ map(map(f)))) ∘ map(g)     🝖-[ [≡]-with2ₗ(_∘_)(_) ([≡]-with2ᵣ(_∘_)(_) (NaturalTransformation.natural([∃]-proof Μ))) ]
...
BinaryOperator.congruence (Category.binaryOperator monad-category {x}{y}{z}) {f₁}{f₂} f₁f₂ {g₁}{g₂} g₁g₂ =
  ...
  μ(z) ∘ (map f₁ ∘ g₁) 🝖-[ [≡]-with2ᵣ(_∘_)(μ(z)) ([≡]-with2(_∘_) ([≡]-with(map) f₁f₂) g₁g₂) ]
...
Morphism.Associativity.proof (Category.associativity monad-category) {x} {y} {z} {w} {f} {g} {h} =
  ext(ext(f) ∘ g) ∘ h   🝖-[ [≡]-with2ₗ(_∘_)(_) (ExtensionSystem.ext-distribute extension-system) ]
  (ext(f) ∘ ext(g)) ∘ h 🝖-[ Morphism.associativity(_∘_) ]
  ext(f) ∘ (ext(g) ∘ h) 🝖-end
Morphism.Identityₗ.proof (Tuple.left (Category.identity monad-category)) {x} {y} {f} =
  ext(η(y)) ∘ f          🝖[ _≡_ ]-[]
  (μ(y) ∘ map(η(y))) ∘ f 🝖-[ [≡]-with2ₗ(_∘_)(f) (_⊜_.proof μ-functor-[∘]-identityₗ) ]
  id ∘ f                 🝖-[ Morphism.identityₗ(_∘_)(id) ]
  f                      🝖-end
