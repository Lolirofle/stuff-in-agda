  ...
  module _ ⦃ classical : ∀{ℓ}{P : Stmt{ℓ}} → Classical(P) ⦄ where
    ...
    closure-membership{E = E}{p = p} = [↔]-intro left closure-membershipᵣ where
      ...
      left overl = [¬¬]-elim ⦃ classical ⦄ $ \npCl →
        ...
        ((p ∉ E) ∧ ∃(r ↦ E ⊆ ∁(PuncturedNeighborhood(p)(r))))           ⇒-[ ((\([∧]-intro npE ([∃]-intro r ⦃ EPN ⦄)) → let [∃]-intro q ⦃ [∧]-intro dpqr qE ⦄ = overl{r} in EPN qE ([¬¬]-elim ⦃ classical ⦄ (Either.elim ([≥][≢]-to-[>] (NonNegative.proof non-negativity) (contrapositiveᵣ([↔]-to-[→] self-distance) (npE ∘ (pq ↦ substitute₁(E) (symmetry(_≡_) pq) qE)))) dpqr ∘ Either.map [¬¬]-elim [¬¬]-elim ∘ [¬]-preserves-[∧][∨]ᵣ ⦃ classical ⦄ ⦃ classical ⦄)))) ]
        ⊥                                                                 ⇒-end
