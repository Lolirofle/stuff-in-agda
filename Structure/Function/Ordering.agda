...
module _ {ℓₒ₁ ℓₒ₂ ℓₗ₁ ℓₗ₂} {T₁ : Type{ℓₒ₁}} {T₂ : Type{ℓₒ₂}} (_≤₁_ : T₁ → T₁ → Type{ℓₗ₁}) (_≤₂_ : T₂ → T₂ → Type{ℓₗ₂}) where
  record Increasing (f : T₁ → T₂) : Stmt{ℓₒ₁ ⊔ ℓₗ₁ ⊔ ℓₗ₂} where
    constructor intro
    field proof : ∀{a b : T₁} → (a ≤₁ b) → (f(a) ≤₂ f(b))

  record Decreasing (f : T₁ → T₂) : Stmt{ℓₒ₁ ⊔ ℓₗ₁ ⊔ ℓₗ₂} where
    constructor intro
  ...
  record UpperBoundPointOf (f : T₁ → T₂) (p : T₁) : Stmt{ℓₒ₁ ⊔ ℓₗ₂} where
    constructor intro
  ...
  record LowerBoundPointOf (f : T₁ → T₂) (p : T₁) : Stmt{ℓₒ₁ ⊔ ℓₗ₂} where
    constructor intro
