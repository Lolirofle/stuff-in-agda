...
module _ {ℓₒ₁}{ℓₒ₂} {A : Type{ℓₒ₁}} ⦃ _ : Equiv(A) ⦄ {B : Type{ℓₒ₂}} ⦃ _ : Equiv(B) ⦄ (f : A → B) where
  ...
  injective-to-unique (Injective.intro(inj)) {y} {x₁}{x₂} fx₁y fx₂y =
    inj{x₁}{x₂} (fx₁y 🝖 symmetry(_≡_) fx₂y)

  instance
    bijective-to-injective : ⦃ _ : Bijective(f) ⦄ → Injective(f)
    Injective.proof(bijective-to-injective ⦃ Bijective.intro(bij) ⦄) {x₁}{x₂} (fx₁fx₂) =
  ...
  instance
    ...
    Surjective.proof(bijective-to-surjective ⦃ Bijective.intro(bij) ⦄) {y} =
      [∃!]-existence (bij {y})

  instance
    injective-surjective-to-bijective : ⦃ _ : Injective(f) ⦄ → ⦃ _ : Surjective(f) ⦄ → Bijective(f)
    Bijective.proof(injective-surjective-to-bijective ⦃ inj ⦄ ⦃ Surjective.intro(surj) ⦄) {y} =
