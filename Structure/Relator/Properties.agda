...
module _ {T : Type{ℓ₁}} (_▫_ : T → T → Stmt{ℓ₂}) where
  ...
  reflexivity = inferArg Reflexivity.proof
...
module _ {T : Type{ℓ₁}} (_▫_ : T → T → Stmt{ℓ₂}) where
  ...
  transitivity = inferArg Transitivity.proof
...
module _ {T : Type{ℓ₁}} (_▫₁_ : T → T → Stmt{ℓ₂}) (_▫₂_ : T → T → Stmt{ℓ₃}) where
  ...
  antisymmetry = inferArg Antisymmetry.proof
...
module _ {T : Type{ℓ₁}} (_▫_ : T → T → Stmt{ℓ₂}) where
  ...
  irreflexivity = inferArg Irreflexivity.proof
...
module _ {T : Type{ℓ₁}} (_▫_ : T → T → Stmt{ℓ₂}) where
  ...
  converseTotal = inferArg ConverseTotal.proof
...
module _ {T : Type{ℓ₁}} (_▫_ : T → T → Stmt{ℓ₂}) where
  ...
  symmetry = inferArg Symmetry.proof
-- {x y : T} → (x ▫ y) → (y ▫ x)
...
module _ {T : Type{ℓ₁}} (_▫_ : T → T → Stmt{ℓ₂}) where
  ...
  asymmetry = inferArg Asymmetry.proof
-- {x y : T} → (x ▫ y) → ¬(y ▫ x)
...
module _ (_▫₁_ : A → B → Stmt{ℓ₁}) (_▫₂_ : A → A → Stmt{ℓ₂}) where
  ...
  subtransitivityₗ = inferArg Subtransitivityₗ.proof
...
module _ (_▫₁_ : A → B → Stmt{ℓ₁}) (_▫₂_ : B → B → Stmt{ℓ₂}) where
  ...
  subtransitivityᵣ = inferArg Subtransitivityᵣ.proof
...
module _ {T : Type{ℓ₁}} (_▫_ : T → T → Stmt{ℓ₂}) where
  ...
  cotransitivity = inferArg CoTransitivity.proof
