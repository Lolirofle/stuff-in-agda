...
-- [∣ᵣₑₘ]-remainder-dividend : ∀{x y}{r : 𝕟(y)} → (x < y) → (y ∣ᵣₑₘ x)(r) → (x ≡ toℕ r)
...
[∣ᵣₑₘ]-is-division-with-remainder : ∀{x y}{r} → (p : (y ∣ᵣₑₘ x)(r)) → ((([∣ᵣₑₘ]-quotient p) ⋅ y) + (toℕ ([∣ᵣₑₘ]-remainder p)) ≡ x)
...
[∣ᵣₑₘ]-is-division-with-remainder {𝐒 .(x + y)}    {𝐒 y} {𝟎}   (DivRem𝐒 {x = x} p) =
  ...
  𝐒(x + y)                            🝖-end
[∣ᵣₑₘ]-is-division-with-remainder {𝐒 .(toℕ r)} {𝐒 y} {𝐒 r} DivRem𝟎 = [≡]-intro
[∣ᵣₑₘ]-is-division-with-remainder {𝐒 .(x + y)} {𝐒(y@(𝐒 _))} {r@(𝐒 _)} (DivRem𝐒 {x = x} p) =
  (([∣ᵣₑₘ]-quotient (DivRem𝐒 p)) ⋅ 𝐒(y)) + (toℕ r) 🝖[ _≡_ ]-[]
  (𝐒([∣ᵣₑₘ]-quotient p) ⋅ 𝐒(y))          + (toℕ r) 🝖[ _≡_ ]-[ congruence₂-₁(_+_)(toℕ r) ([⋅]-with-[𝐒]ₗ {[∣ᵣₑₘ]-quotient p}{𝐒(y)}) ]
  ((([∣ᵣₑₘ]-quotient p) ⋅ 𝐒(y)) + 𝐒(y))  + (toℕ r) 🝖[ _≡_ ]-[ One.commuteᵣ-assocₗ {a = ([∣ᵣₑₘ]-quotient p) ⋅ 𝐒(y)}{b = 𝐒(y)}{c = toℕ r} ]
  ((([∣ᵣₑₘ]-quotient p) ⋅ 𝐒(y)) + (toℕ r)) + 𝐒(y)  🝖[ _≡_ ]-[]
  𝐒(((([∣ᵣₑₘ]-quotient p) ⋅ 𝐒(y)) + (toℕ r)) + y)  🝖[ _≡_ ]-[ congruence₁(𝐒) (congruence₂-₁(_+_)(y) ([∣ᵣₑₘ]-is-division-with-remainder p)) ]
  𝐒(x + y)                                                 🝖-end 
...
[∣ᵣₑₘ]-equivalence : ∀{x y}{r} → (y ∣ᵣₑₘ x)(r) ↔ ∃(q ↦ (q ⋅ y) + (toℕ r) ≡ x)
[∣ᵣₑₘ]-equivalence = [↔]-intro (p ↦ l {q = [∃]-witness p} ([∃]-proof p)) (p ↦ [∃]-intro ([∣ᵣₑₘ]-quotient p) ⦃ [∣ᵣₑₘ]-is-division-with-remainder p ⦄) where
  l :  ∀{x y q}{r} → ((q ⋅ y) + (toℕ r) ≡ x) → (y ∣ᵣₑₘ x)(r)
  l {_}{_}{𝟎}  {_} [≡]-intro = DivRem𝟎
  l {x}{y}{𝐒 q}{r} p = substitute₁ᵣ(x ↦ (y ∣ᵣₑₘ x)(r)) eq (DivRem𝐒 (l{(q ⋅ y) + (toℕ r)}{y}{q}{r} [≡]-intro)) where
    eq =
      ((q ⋅ y) + (toℕ r)) + y 🝖[ _≡_ ]-[ One.commuteᵣ-assocₗ {a = q ⋅ y}{b = toℕ r}{c = y} ]
      ((q ⋅ y) + y) + (toℕ r) 🝖[ _≡_ ]-[ congruence₂-₁(_+_)(toℕ r) ([⋅]-with-[𝐒]ₗ {q}{y}) ]-sym
      (𝐒(q) ⋅ y) + (toℕ r)    🝖[ _≡_ ]-[ p ]
      x                          🝖-end
...
∃.witness ([∣ᵣₑₘ]-existence {x} {y}) = fromℕ (x mod 𝐒(y)) ⦃ {![↔]-to-[→] decide-is-true mod-maxᵣ!} ⦄
∃.proof   ([∣ᵣₑₘ]-existence {x} {y}) = [↔]-to-[←] [∣ᵣₑₘ]-equivalence ([∃]-intro (x ⌊/⌋ 𝐒(y)) ⦃ {!TODO: Insert division theorem here!} ⦄)
-}

DivRem𝟎Alt : ∀{x y} → (xy : (x < y)) → (y ∣ᵣₑₘ x)(fromℕ x ⦃ [↔]-to-[→] decider-true xy ⦄)
DivRem𝟎Alt {x} {𝐒 y} (succ p) = substitute₁ᵣ(expr ↦ (𝐒 y ∣ᵣₑₘ expr)(fromℕ x)) (𝕟.𝕟-ℕ-inverse) ((DivRem𝟎{𝐒(y)}{fromℕ x})) where
  instance
    x<𝐒y : IsTrue (x <? 𝐒(y))
...
[∣ᵣₑₘ]-existence-alt : ∀{x y} ⦃ pos : Positive(y) ⦄ → ∃(y ∣ᵣₑₘ x)
[∣ᵣₑₘ]-existence-alt {x}{𝐒 y} = ℕ-strong-induction(x ↦ ∃(𝐒(y) ∣ᵣₑₘ x)) base step x where
  base : ∃(𝐒(y) ∣ᵣₑₘ 𝟎)
  base = [∃]-intro 𝟎 ⦃ DivRem𝟎 ⦄

  step : (i : ℕ) → ((j : ℕ) → (j ≤ i) → ∃(𝐒(y) ∣ᵣₑₘ j)) → ∃(𝐒(y) ∣ᵣₑₘ 𝐒(i))
  step i p with [≤][>]-dichotomy {y}{i}
  ... | [∨]-introₗ yi = [∃]-map-proof (DivRem𝐒Alt([≤]-with-[𝐒] ⦃ yi ⦄)) (p(𝐒(i) −₀ 𝐒(y)) ([−₀]-lesser {i}{y}))
  ... | [∨]-introᵣ 𝐒iy = [∃]-intro (fromℕ (𝐒(i)) ⦃ [↔]-to-[→] decider-true 𝐒iy ⦄) ⦃ DivRem𝟎Alt ([≤]-with-[𝐒] ⦃ 𝐒iy ⦄) ⦄

{-
open import Structure.Setoid.Uniqueness
{-[∣ᵣₑₘ]-uniqueness : ∀{x y}{p : ∃(𝐒(y) ∣ᵣₑₘ x)} → (p ≡ [∣ᵣₑₘ]-existence)
[∣ᵣₑₘ]-uniqueness {.(toℕ r)} {y} {[∃]-intro r ⦃ DivRem𝟎 ⦄} = {![≡]-intro!}
[∣ᵣₑₘ]-uniqueness {.(x + 𝐒 y)} {y} {[∃]-intro r ⦃ DivRem𝐒 {x = x} p ⦄} = {!!}-}
{-[∣ᵣₑₘ]-unique-remainder : ∀{x y} → Unique(𝐒(y) ∣ᵣₑₘ x)
[∣ᵣₑₘ]-unique-remainder {.(toℕ a)} {y} {a} {b} DivRem𝟎 q = {!!}
...
[∣ᵣₑₘ]-mereProposition = intro proof where
  ...
  proof {.(toℕ r)} {y} {r} {DivRem𝟎} {q} = {!!}
  proof {.(x + 𝐒 y)} {y} {r} {DivRem𝐒 {x = x} p} {q} = {!p!}

  -- testor : ∀{y x}{r : 𝕟(𝐒 y)}{p : (𝐒(y) ∣ᵣₑₘ x)(r)} → (p ≡ DivRem𝟎) ∨ ∃(q ↦ (p ≡ DivRem𝐒 q))

  -- TODO: Maybe by injectivity of toℕ?
  test : ∀{y}{r : 𝕟(𝐒 y)}{p : (𝐒(y) ∣ᵣₑₘ (toℕ r))(r)} → (p ≡ DivRem𝟎)
{-  test {y} {r} {p} with toℕ r
  test {_} {_} {_} | 𝟎 = {!!}-}
-}
...
[∣ᵣₑₘ]-mereProposition = intro {!!} where
  ...
  proof {y} {r} (intro .(toℕ r) DivRem𝟎) (intro .(toℕ r) DivRem𝟎) = [≡]-intro
  proof {y} {r} (intro .(toℕ r) DivRem𝟎) (intro .(x₂ + y) (DivRem𝐒{x = x₂} p₂)) = {!!}
  proof {y} {r} (intro .(x₁ + y) (DivRem𝐒{x = x₁} p₁)) (intro .(toℕ r) DivRem𝟎) = {!!}
...
{- TODO: When modulo addition in 𝕟 is formalized properly
open import Numeral.Finite as 𝕟
open import Numeral.Natural.Relation

private variable r r₁ r₂ : 𝕟(n)

[+]-of-[∣ᵣₑₘ] : ⦃ pos : Positive(d) ⦄ → (d ∣ᵣₑₘ a)(r₁) → (d ∣ᵣₑₘ b)(r₂) → (d ∣ᵣₑₘ (a + b))(r₁ [+] r₂)
[+]-of-[∣ᵣₑₘ] {𝐒 d} da DivRem𝟎 = da
[+]-of-[∣ᵣₑₘ] {𝐒 d} da (DivRem𝐒 db) = DivRem𝐒 {![+]-of-[∣ᵣₑₘ] {𝐒 d} da db!}
-}
