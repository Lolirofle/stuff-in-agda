...
module _ where
  ...
  instance
    ...
    Equivalence.transitivity (Equiv.equivalence ℚ₊₀-equiv) = crossMul-transitivity (\{x}{y}{z} → [↔]-intro ([⋅]-cancellationᵣ {z}{x}{y}) (congruence₂-₁(ℕ._⋅_)(z)))
  ...
  ℚ-substitute-divides {x₁}{x₂}{y₁@(𝐒 _)}{y₂} eq div =
    let
      p =
        ...
        (x₁ ℕ.⋅ y₂) ℕ.⌊/⌋₀ y₁ 🝖[ Id ]-[ congruence₂-₁(ℕ._⌊/⌋₀_)(y₁) eq ]
...
module _ where
  ...
  instance
    ...
    Associativity.proof [+]-associativity {x₁ /ₙ y₁} {x₂ /ₙ y₂} {x₃ /ₙ y₃} = congruence₂(ℕ._⋅_) l r where
      open import Relator.Equals.Proofs.Equiv
      l =
        (((x₁ ℕ.⋅ y₂) ℕ.+ (y₁ ℕ.⋅ x₂)) ℕ.⋅ y₃) ℕ.+ ((y₁ ℕ.⋅ y₂) ℕ.⋅ x₃)          🝖[ Id ]-[ congruence₂-₁(ℕ._+_)((y₁ ℕ.⋅ y₂) ℕ.⋅ x₃) (distributivityᵣ(ℕ._⋅_)(ℕ._+_) {x₁ ℕ.⋅ y₂}{y₁ ℕ.⋅ x₂}{y₃}) ]
        ...
        (x₁ ℕ.⋅ (y₂ ℕ.⋅ y₃)) ℕ.+ ((y₁ ℕ.⋅ (x₂ ℕ.⋅ y₃)) ℕ.+ (y₁ ℕ.⋅ (y₂ ℕ.⋅ x₃))) 🝖[ Id ]-[ congruence₂-₂(ℕ._+_)(x₁ ℕ.⋅ (y₂ ℕ.⋅ y₃)) (distributivityₗ(ℕ._⋅_)(ℕ._+_) {y₁}{x₂ ℕ.⋅ y₃}{y₂ ℕ.⋅ x₃}) ]-sym
  ...
  instance
    ...
    Distributivityₗ.proof [⋅][+]-distributivityₗ {x₁ /ₙ y₁} {x₂ /ₙ y₂} {x₃ /ₙ y₃} =
      ...
      (((x₁ ℕ.⋅ (x₂ ℕ.⋅ y₃)) ℕ.+ (x₁ ℕ.⋅ (y₂ ℕ.⋅ x₃))) ℕ.⋅ y₁) ℕ.⋅ (y₁ ℕ.⋅ (y₂ ℕ.⋅ y₃))        🝖[ Id ]-[ congruence₂-₁(ℕ._⋅_)(y₁ ℕ.⋅ (y₂ ℕ.⋅ y₃)) p ]
      ...
      where
        ...
        p =
          ...
          ((x₁ ℕ.⋅ (x₂ ℕ.⋅ y₃)) ℕ.⋅ y₁) ℕ.+ ((x₁ ℕ.⋅ (y₂ ℕ.⋅ x₃)) ℕ.⋅ y₁) 🝖[ Id ]-[ congruence₂(ℕ._+_) (One.associate4-213-222 {_▫_ = ℕ._⋅_} {x₁}{x₂}{y₃}{y₁} 🝖 congruence₂-₂(ℕ._⋅_)(x₁ ℕ.⋅ x₂) (commutativity(ℕ._⋅_) {y₃}{y₁})) (commutativity(ℕ._⋅_) {x₁ ℕ.⋅ (y₂ ℕ.⋅ x₃)}{y₁} 🝖 One.associate4-321-222 {_▫_ = ℕ._⋅_} {y₁}{x₁}{y₂}{x₃} 🝖 One.associate-commute4-c {_▫_ = ℕ._⋅_} {y₁}{x₁}{y₂}{x₃}) ]
  ...
  instance
    ...
    InverseOperatorᵣ.proof [+][−₀]-inverseOperatorᵣ {x₁ /ₙ x₂} {y₁ /ₙ y₂} =
      ((((x₁ ℕ.⋅ y₂) ℕ.+ (x₂ ℕ.⋅ y₁)) ℕ.⋅ y₂) ℕ.−₀ ((x₂ ℕ.⋅ y₂) ℕ.⋅ y₁)) ℕ.⋅ x₂ 🝖[ Id ]-[ congruence₂-₁(ℕ._⋅_)(x₂) (congruence₂-₂(ℕ._−₀_)(((x₁ ℕ.⋅ y₂) ℕ.+ (x₂ ℕ.⋅ y₁)) ℕ.⋅ y₂) (One.commuteᵣ-assocₗ{_▫_ = ℕ._⋅_} {x₂}{y₂}{y₁})) ]
      ((((x₁ ℕ.⋅ y₂) ℕ.+ (x₂ ℕ.⋅ y₁)) ℕ.⋅ y₂) ℕ.−₀ ((x₂ ℕ.⋅ y₁) ℕ.⋅ y₂)) ℕ.⋅ x₂ 🝖[ Id ]-[ congruence₂-₁(ℕ._⋅_)(x₂) (distributivityᵣ(ℕ._⋅_)(ℕ._−₀_) {(x₁ ℕ.⋅ y₂) ℕ.+ (x₂ ℕ.⋅ y₁)}{x₂ ℕ.⋅ y₁}{y₂}) ]-sym
      ((((x₁ ℕ.⋅ y₂) ℕ.+ (x₂ ℕ.⋅ y₁)) ℕ.−₀ (x₂ ℕ.⋅ y₁)) ℕ.⋅ y₂) ℕ.⋅ x₂          🝖[ Id ]-[ congruence₂-₁(ℕ._⋅_)(x₂) (congruence₂-₁(ℕ._⋅_)(y₂) (inverseOperᵣ(ℕ._+_)(ℕ._−₀_) {x₁ ℕ.⋅ y₂}{x₂ ℕ.⋅ y₁})) ]
      ((x₁ ℕ.⋅ y₂) ℕ.⋅ y₂) ℕ.⋅ x₂                                               🝖[ Id ]-[ One.commuteᵣ-assocₗ{_▫_ = ℕ._⋅_} {x₁ ℕ.⋅ y₂}{y₂}{x₂} ]
      ((x₁ ℕ.⋅ y₂) ℕ.⋅ x₂) ℕ.⋅ y₂                                               🝖[ Id ]-[ congruence₂-₁(ℕ._⋅_)(y₂) (One.commuteᵣ-assocₗ{_▫_ = ℕ._⋅_} {x₁}{y₂}{x₂}) ]
  ...
  instance
    ...
    Commutativity.proof [+]-commutativity {x₁ /ₙ x₂} {y₁ /ₙ y₂} =
      ...
      ((x₂ ℕ.⋅ y₁) ℕ.+ (x₁ ℕ.⋅ y₂)) ℕ.⋅ (x₂ ℕ.⋅ y₂) 🝖[ Id ]-[ congruence₂-₁(ℕ._⋅_)(x₂ ℕ.⋅ y₂) (congruence₂(ℕ._+_) (commutativity(ℕ._⋅_) {x₂}{y₁}) (commutativity(ℕ._⋅_) {x₁}{y₂})) ]
