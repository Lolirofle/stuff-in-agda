...
open import Relator.Equals.Proofs.Equiv using () renaming ([≡]-equiv to Id-equiv ; [≡]-function to Id-function)
...
module Interleaving where
  ...
  split {𝐒 a} {𝐒 b} (𝐒(𝐒(n))) = Either.map 𝐒 𝐒 (split {a} {b} n)
  ...
  instance
    split-join-inverses : Inverseₗ(join{a}{b})(split{a}{b})
    split-join-inverses {a}{b} = intro(proof{a}{b}) where
      ...
      ... | 𝟎      | p = congruence₁(Either.map 𝐒 𝐒) p
      ... | 𝐒(𝐒 m) | p = congruence₁(Either.map 𝐒 𝐒) p
      proof {𝐒(𝐒 a)} {𝐒(𝐒 b)} {Either.Right (𝐒 n)} with join{𝐒 a}{𝐒 b} (Either.Right n) | proof {𝐒 a}{𝐒 b}{Either.Right n}
      ... | 𝐒(𝟎)   | p = congruence₁(Either.map 𝐒 𝐒) p
      ... | 𝐒(𝐒 m) | p = congruence₁(Either.map 𝐒 𝐒) p

  instance
    split-join-inverse : Inverse(split{a}{b})(join{a}{b})
    split-join-inverse = [∧]-intro join-split-inverses split-join-inverses

  instance
    split-bijective : Bijective(split{a}{b})
    split-bijective = invertible-to-bijective ⦃ inver = [∃]-intro join ⦃ [∧]-intro Id-function split-join-inverse ⦄ ⦄
...
interleave af bf = Either.map af bf ∘ Interleaving.split

module _
  ⦃ equiv-A : Equiv{ℓₑ₁}(A) ⦄
  ⦃ equiv-B : Equiv{ℓₑ₂}(B) ⦄
  ⦃ equiv-AB : Equiv{ℓₑ}(A ‖ B) ⦄ ⦃ ext-AB : Either.Extensionality ⦃ equiv-A ⦄ ⦃ equiv-B ⦄ (equiv-AB) ⦄
  {af : 𝕟(a) → A} ⦃ bij-af : Bijective ⦃ Id-equiv ⦄ ⦃ equiv-A ⦄ (af) ⦄
  {bf : 𝕟(b) → B} ⦃ bij-bf : Bijective ⦃ Id-equiv ⦄ ⦃ equiv-B ⦄ (bf) ⦄
  where

  instance
    interleave-bijective : Bijective(interleave af bf)
    interleave-bijective = [∘]-bijective ⦃ bij-f = Either.map-bijective ⦄ ⦃ bij-g = Interleaving.split-bijective ⦄
...
concat af bf = Either.map af bf ∘ Concatenation.split
...
interleave-join-equality : ∀{af : 𝕟(a) → A}{bf : 𝕟(b) → B} → (interleave af bf ∘ Interleaving.join ⊜ Either.map af bf)
interleave-join-equality {a = a}{b = b} = intro(p{a = a}{b = b}) where
  p : ∀{a b}{af : 𝕟(a) → A}{bf : 𝕟(b) → B} → (interleave af bf ∘ Interleaving.join Names.⊜ Either.map af bf)
...
instance
  ...
  interleave-injective {a = a}{b = b}{af = af}{bf = bf} = substitute₁ₗ(Injective) (interleave-split-equality {af = af}{bf = bf}) ([∘]-injective {f = Either.map af bf}{g = Interleaving.split} ⦃ inj-g = inverse-to-injective ⦃ inver = [∧]-intro Interleaving.join-split-inverse Interleaving.split-join-inverse ⦄ ⦄)

instance
  interleave-surjective : ∀{af : 𝕟(a) → A}{bf : 𝕟(b) → B} → ⦃ Surjective(af) ⦄ → ⦃ Surjective(bf) ⦄ → Surjective(interleave af bf)
  interleave-surjective {a = a}{b = b}{af = af}{bf = bf} = substitute₁ₗ(Surjective) (interleave-split-equality {af = af}{bf = bf}) ([∘]-surjective {f = Either.map af bf}{g = Interleaving.split} ⦃ surj-g = inverse-to-surjective ⦃ inver = [∧]-intro Interleaving.join-split-inverse Interleaving.split-join-inverse ⦄ ⦄)

instance
  interleave-bijective : ∀{af : 𝕟(a) → A}{bf : 𝕟(b) → B} → ⦃ Bijective(af) ⦄ → ⦃ Bijective(bf) ⦄ → Bijective(interleave af bf)
  interleave-bijective {a = a}{b = b}{af = af}{bf = bf} = substitute₁ₗ(Bijective) (interleave-split-equality {af = af}{bf = bf}) ([∘]-bijective {f = Either.map af bf}{g = Interleaving.split} ⦃ bij-g = inverse-to-bijective ⦃ inver = [∧]-intro Interleaving.join-split-inverse Interleaving.split-join-inverse ⦄ ⦄)

concat-split-equality : ∀{af : 𝕟(a) → A}{bf : 𝕟(b) → B} → (concat af bf ⊜ Either.map af bf ∘ Concatenation.split)
concat-split-equality {a = a}{b = b} = intro(p{a = a}{b = b}) where
  p : ∀{a b}{af : 𝕟(a) → A}{bf : 𝕟(b) → B} → (concat af bf Names.⊜ Either.map af bf ∘ Concatenation.split)
