...
data List {lvl} (n : ℕ) (T : Type{lvl}) : Type{lvl} where
...
toList : ∀{lvl}{T : Type{lvl}} → (List 1 T) → (NormalList.List T)
toList (End x) = x NormalList.⊰ NormalList.∅
toList (x ⊰ l) = x NormalList.⊰ (toList l)

fromList : ∀{lvl}{T : Type{lvl}} → (NormalList.List T) → T → (List 1 T)
...
map : ∀{lvl}{T₁ T₂ : Type{lvl}} → (T₁ → T₂) → (List 1 T₁) → (List 1 T₂)
map f (End x) = End(f x)
map f (elem ⊰ l) = (f elem) ⊰ (map f l)

reduceₗ : ∀{lvl}{T : Type{lvl}} → (T → T → T) → (List 1 T) → T
reduceₗ _   (End x) = x
reduceₗ _▫_ (elem ⊰ l) = NormalList.foldₗ _▫_ elem (toList l)

reduceᵣ : ∀{lvl}{T : Type{lvl}} → (T → T → T) → (List 1 T) → T
reduceᵣ _   (End x) = x
reduceᵣ _▫_ (elem ⊰ l) = elem ▫ (reduceᵣ _▫_ l)

index : ∀{lvl}{T : Type{lvl}} → ℕ → (List 1 T) → (Option T)
index 0      (End x) = Option.Some(x)
index 0      (x ⊰ _) = Option.Some(x)
index (𝐒(n)) (_ ⊰ l) = index n l
index _      _       = Option.None

mapWindow2ₗ : ∀{lvl₁ lvl₂}{T : Type{lvl₁}}{R : Type{lvl₂}} → (T → T → R) → (List 1 T) → (NormalList.List R)
...
firstElem : ∀{lvl}{T : Type{lvl}} → (List 1 T) → T
firstElem (End x) = x
firstElem (x ⊰ _) = x

lastElem : ∀{lvl}{T : Type{lvl}} → (List 1 T) → T
lastElem (End x) = x
